...
export default function useGrupos() {
    ...
    useEffect(() => {
        onAuthStateChanged(auth, () => {
            ...
            if (auth.currentUser) {
                ...
                setError({ status: false, msg: false });
    ...
    const createNewGrupo = useCallback(async (grupo) => {
        if (grupos.length >= 10) {
            ...
            addDoc(colRef, { ...grupo, codinv: codigoInv, users: [auth.currentUser.uid], categories: defaultCategories })
                .then((res) => {
                    setSuccess({
                        status: true,
                        msg: 'Grupo creado con exito!',
                    });
                    setTimeout(() => {
                        history.push('/groups');
                    }, 2000);
    ...
    const checkUserinGroup = async (uiduser, groupcode) => {
        const colRef = collection(db, 'grupos');
        let ok = false;
        const q = query(colRef, where('users', 'array-contains', uiduser), where('codinv', '==', groupcode));
        const querySnapshot = await getDocs(q);
        if (querySnapshot.empty) {
            ok = false;
        } else {
    ...
    const getGroup = useCallback(async (uid, ref = true) => {
        ...
        if (ref && Grupo.data().gastos) {
            Grupo.data().gastos.forEach(async (gasto) => {
                ...
                getDoc(docRef).then((profile) => {
                    gasto.profileData = profile.data();
                    gasto.fechaConvert = new Date(gasto.fecha).toLocaleDateString('es-ES');
                    gastosArray.push(gasto);
                });
            });
            if (isSubscribed) {
    ...
    const getGrupoGastosTotales = useCallback(async () => {
        setLoadingGrupos(true);
        const colRefGrupo = collection(db, 'grupos');
        const q = query(colRefGrupo, where('users', 'array-contains', auth.currentUser.uid), where('onlyread', '==', true));
        const querySnapshot = await getDocs(q);
