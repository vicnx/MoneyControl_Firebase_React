...
const MClistcuentas = (props) => {
    const { cuentas, loadingcuentas, deleteCuenta, success, setSuccess, error, setError } = useCuentas();
    ...
    return (
        <>
            {loadingcuentas ? (
                <ClipLoader color={'blue'} loading={true} css={'display: block;margin: 0 auto'} size={150} />
            ) : cuentas ? (
                <IonList>
                    {cuentas.map((c, index) => (
                        <IonItem className="cuenta-item" key={index}>
                            ...
                            <CountUp end={c.cantidad} suffix=" €" duration={1.5} separator=" " decimal="," decimals={2} className="contador-money" />
                            ...
                            <div className="cuenta-options" slot="end">
                                <div
                                    ...
                                    <DynamicFaIcon name="trash" color="var(--ion-color-danger-tint)" />
                                </div>
                            </div>
                        </IonItem>
                    ))}
            ...
            <IonAlert
                ...
                message={'¿Seguro que quieres elimiar la cuenta</br><strong>' + selected.name + '</strong>?'}
                buttons={[
                    {
                        text: 'Cancelar',
                        role: 'cancel',
